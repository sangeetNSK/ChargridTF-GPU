...
te.pytesseract.tesseract_cmd = r'C:\Users\U1092599\AppData\Local\Programs\Tesseract-OCR\tesseract.exe'

## Hyperparameters
#dir_img = os.getenv('DIR_IMG')
dir_img= 'C:\\Users\\u1092599\\Desktop\\chargridDemo\\chargridMin\\data\\img'
dir_boxes='C:\\Users\\u1092599\\Desktop\\chargridDemo\\chargridMin\\data\\box'
#dir_boxes = os.getenv('DIR_BOXES')
dir_classes ='C:\\Users\\u1092599\\Desktop\\chargridDemo\\chargridMin\\data\\key'
...
def get_chargrid(dt):
    ...
    for index, row in dt.iterrows():
        for i in range(0, len(row["text"])):
            row['width'] = (row['width'] + len(row["text"]) - 1) // len(row["text"]) * len(
                row["text"])  # Split character by character
...
def extract_class_bounding_boxes(filename):
    ...
    pd_boxes = pd.DataFrame(columns=['top_left_x', 'top_left_y', 'top_right_x', 'top_right_y', 'bot_left_x',
                                     'bot_left_y', 'bot_right_x', 'bot_right_y', 'text'])
    dic_class = dict()

    with open(os.path.join(dir_boxes,filename).replace("jpg", "csv")) as f:
        reader = f.read().splitlines()
        pd_boxes = pd.DataFrame([x.split(",", 8) for x in reader],
                                columns=['top_left_x', 'top_left_y', 'top_right_x', 'top_right_y', 'bot_right_x',
                                         'bot_right_y', 'bot_left_x', 'bot_left_y', 'text'])
    ...
    vectorized_text = CountVectorizer().fit_transform(
        [dic_class[list_classes[i]] for i in range(nb_classes)] + pd_boxes["text"].tolist())
